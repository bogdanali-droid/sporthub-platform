...
export async function POST({ params, request, locals }: APIContext) {
  ...
  await execute(db, `
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    visitId, id, locals.user?.club_id, body.visit_date, body.valid_until ?? null,
    ...
    body.notes ?? null, locals.user?.id
  ]);
...
export async function DELETE({ request, locals }: APIContext) {
  ...
  const db = (locals as any).runtime?.env?.DB;
  const body = await request.json() as any;
  await execute(db, 'DELETE FROM medical_visits WHERE id = ? AND club_id = ?', [body.visit_id, locals.user?.club_id]);
