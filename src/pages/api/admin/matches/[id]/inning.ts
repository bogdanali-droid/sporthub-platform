...
export const POST: APIRoute = async ({ locals, params, request }) => {
  ...
  const match = await queryFirst<any>(db, `SELECT * FROM matches WHERE id=? AND club_id=?`, [id, locals.user.club_id]);
  ...
  await execute(db,
    `UPDATE matches SET period_data=?, score_home=?, score_away=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
    [JSON.stringify(periods), scoreHome, scoreAway, id]);
